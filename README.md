# Laravel5.8公式ドキュメント翻訳リポジトリ

このリポジトリはPHP Webアプリフレームワーク、Laravel5.8の公式英文ドキュメントを日本語へ翻訳しています。

This is a Japanese translation repository for Laravel 5.8 official documentations.

翻訳内容は、<https://readouble.com/laravel/5.8/ja>で公開しています。
...
* このリポジトリでは5.8の翻訳和文のみ管理しています。readouble.comで公開している他のバージョンの和文に関しては、@HiroKwsにツイートいただくか、hirokws@gmail.comへご連絡ください。なお、メンテナンス期間をフレームワーク本体と合わせているため、readouble.comで公開している日本語翻訳の旧バージョンは更新していません。バージョン5.8リリース時点で、メンテ対象の和文バージョンは5.5と5.8のみです。

#### issueで報告する場合

Issueで報告していただくのが、一番簡単です。

<https://github.com/laravel-ja/ja-docs-5.8/issues>のページで、"New issue"ボタンをクリックし、issueを発行してご報告ください。
...
原則、Laravel5.8のメンテ期間である半年間。次回のバージョンのリリースまで。２０１９年８月頃までの予定です。
