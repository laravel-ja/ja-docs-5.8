...
> {tip} **さっそく取り掛かりたいですか？** 真新しいLaravelアプリケーションで`php artisan make:auth`を実行し、ブラウザで`http://your-app.test/register`か、アプリケーションで割りつけた別のURLへアクセスするだけです。この一つのコマンドで、パスワードリセットを含めた、認証システム全体のスカフォールディングの面倒をみます。
